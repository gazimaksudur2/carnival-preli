...
def analyze_ticket_route(req: AnalyzeRequest):
    provider = _provider_label()
    client = _get_client(provider)
    log.info(
        "analyze_ticket ticket_id=%s lang=%s history=%d provider=%s",
        req.ticket_id, req.language, len(req.transaction_history), provider,
    )
    return analyze_ticket(client, provider, req)
...
async def generic_handler(request: Request, exc: Exception):
    log.error("unhandled_error path=%s err=%s", request.url.path, exc)
